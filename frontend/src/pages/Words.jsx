...
export default function Words() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Kelime eklenemedi (something went wrong)');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Kelime silinemedi (something went wrong)');
    }
  };
      ...
      <main className="max-w-4xl mx-auto px-6 py-8 space-y-6">
        ...
        {showForm && (
          <div className="bg-card rounded-2xl border border-border p-6 shadow-card animate-scale-in">
            <h2 className="text-lg font-bold font-display mb-6">Yeni Kelime Ekle</h2>
        ...
        {words.length === 0 ? (
          <div className="text-center py-20">
            ...
            <h3 className="text-lg font-semibold text-foreground mb-2">Henüz kelime eklenmedi</h3>
            <p className="text-muted-foreground text-sm">Yukarıdaki butona tıklayarak ilk kelimeni ekle!</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-3">
            {words.map((word, i) => (
              <div
                key={word.WordID}
                className={`animate-fade-in-up bg-card rounded-2xl border border-border p-5 shadow-card hover:shadow-card-hover transition-all duration-300 hover:scale-[1.01]`}
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex gap-4 flex-1">
                    {word.Picture && (
                      <img
                        src={`http://localhost:3001${word.Picture}`}
                        alt={word.EngWordName}
                  ...
                  <button
                    ...
                    className="p-2 rounded-xl text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-all duration-300 flex-shrink-0"
                  >
